...
managed_patch=$here/../../windows/provision/qemu-managed-storage.patch
...
# Unlayer the optional managed descriptor patch before checking the base patch.
if git -C "$source" apply --reverse --check "$managed_patch" 2>/dev/null; then git -C "$source" apply --reverse "$managed_patch"; fi
...
git -C "$source" apply "$managed_patch"
cd "$state/qemu-build"
...
sha256sum "$state/qemu-system-x86_64" "$patch" "$managed_patch" > "$state/qemu-fixture.sha256"
